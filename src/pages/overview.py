...
from src.utils.forecast import forecast_monthly, trend_summary
from src.utils.insights import build_insights
...
def render(fdf):
    ...
    fc = forecast_monthly(pd.Series(monthly["Revenue"].values, index=monthly["Month"]))
    takeaway = trend_summary(fc, "revenue (INR)")
    if takeaway:
        st.info(f"🔮 {takeaway}")
    ...
    for insight in build_insights(fdf):
